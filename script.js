...
document.addEventListener('DOMContentLoaded', function() {
    ...
    taxForm.addEventListener('submit', function(e) {
        ...
        const monthsWorkedInput = document.getElementById('months-worked');
        const partsInput = document.getElementById('parts');
        const statusInput = document.getElementById('status');

        const grossSalaryAnnual = parseFloat(grossSalaryInput.value);
        const monthsWorked = parseInt(monthsWorkedInput.value);
        const parts = parseFloat(partsInput.value);
        const status = statusInput.value;

        console.log(`Salaire Brut Annuel Entré : ${grossSalaryAnnual} €`);
        console.log(`Nombre de Mois Travaillés : ${monthsWorked}`);
        console.log(`Nombre de Parts : ${parts}`);
        console.log(`Statut Sélectionné : ${status}`);

        // Validation des entrées
        if (
            isNaN(grossSalaryAnnual) || grossSalaryAnnual < 0 ||
            isNaN(monthsWorked) || monthsWorked < 1 || monthsWorked > 12 ||
            isNaN(parts) || parts <= 0
        ) {
        ...
        // Calcul du salaire brut proportionnel au nombre de mois travaillés
        const grossSalary = (grossSalaryAnnual / 12) * monthsWorked;
        console.log(`Salaire Brut Proportionnel : ${grossSalary.toFixed(2)} €`);
        ...
        console.log(`Taux de Cotisations Sociales : ${(cotisationRate * 100).toFixed(2)}%`);
        ...
        resultDiv.innerHTML = `
            <p><strong>Salaire Brut Annuel Entré :</strong> ${grossSalaryAnnual.toFixed(2)} €</p>
            <p><strong>Nombre de Mois Travaillés :</strong> ${monthsWorked} mois</p>
            <p><strong>Salaire Brut Proportionnel :</strong> ${grossSalary.toFixed(2)} €</p>
